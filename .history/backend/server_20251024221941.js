...
async function initializeDatabase() {
  try {
    ...
    const sampleClasses = [
      ...
      {
        course_id: 4, // MATH101 - Calculus I
        class_name: 'MATH101-A',
        lecturer_id: 2, // John Lecturer
        venue: 'Room 301',
...
app.post('/api/ratings', async (req, res) => {
  ...
  try {
    ...
    // Trim names to normalize
    const trimmedLecturerName = lecturerName.trim();
    const trimmedCourseName = courseName ? courseName.trim() : null;
    ...
    const [existingRating] = await connection.execute(
      ...
      [studentId, trimmedLecturerName, trimmedCourseName]
    );
    ...
    if (existingRating.length > 0) {
      ...
      [result] = await connection.execute(
        `INSERT INTO ratings (student_id, lecturer_name, course_name, rating, review)
         VALUES (?, ?, ?, ?, ?)`,
        [studentId, trimmedLecturerName, trimmedCourseName, rating, review || null]
      );
      res.status(201).json({
        success: true,
...
app.get('/api/ratings/check', async (req, res) => {
  ...
  try {
    ...
    const [existingRating] = await connection.execute(
      ...
      [student_id, lecturer_id, trimmedCourseName]
    );
...
app.put('/api/classes/:id', async (req, res) => {
  ...
  try {
    ...
    const [result] = await connection.execute(
      `UPDATE classes SET
        course_id = ?, class_name = ?, lecturer_id = ?, venue = ?, scheduled_time = ?, total_registered_students = ?, updated_at = CURRENT_TIMESTAMP
        WHERE class_id = ?`,
      [course_id, class_name, lecturer_id, venue || null, scheduled_time, total_registered_students || 0, id]
    );
...
app.get('/api/feedback/report/:reportId', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to fetch report feedback: ' + error.message
    });
  } finally {
    if (connection) connection.release();
  }
...
app.post('/api/feedback', authenticateToken, async (req, res) => {
  let connection;
  try {
    const { report_id, feedback_text, action_items, priority } = req.body;
    ...
    const [result] = await connection.execute(
      `INSERT INTO feedback (report_id, reviewer_id, feedback_text, action_items, priority)
       VALUES (?, ?, ?, ?, ?)`,
      [report_id, req.user.id, feedback_text, action_items || null, priority || 'medium']
    );
