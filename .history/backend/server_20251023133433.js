...
app.get('/api/ratings/check', async (req, res) => {
  let connection;
  try {
    const { lecturer_id, student_id } = req.query;

    if (!lecturer_id || !student_id) {
      return res.status(400).json({
        success: false,
        message: 'Lecturer ID and Student ID are required'
      });
    }

    connection = await pool.getConnection();

    // Check if rating exists for this student and lecturer
    const [existingRating] = await connection.execute(
      'SELECT rating_id FROM ratings WHERE student_id = ? AND lecturer_name = (SELECT full_name FROM users WHERE user_id = ?)',
