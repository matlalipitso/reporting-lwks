...
const PrincipalLecturerDashboard = ({ user, onLogout }) => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      if (generalReportsResult.success) setGeneralReports(generalReportsResult.reports || []);
              ...
              <div className="list-container">
                ...
                {[...lectureReports, ...generalReports].map(report => (
                  <div key={report.id} className="list-row">
                    ...
                    <span>{report.lecturer_name || report.author_name}</span>
                    <span>{report.course_name || 'N/A'}</span>
                    <span>{report.date_of_lecture || report.created_at}</span>
                    <span>
