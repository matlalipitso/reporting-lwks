...
const PrincipalLecturerDashboard = ({ user, onLogout }) => {
  ...
  const handleViewReport = async (report) => {
    ...
    // Load feedback for this report
    try {
      const feedbackResult = await feedbackService.getFeedbackByReportId(report.id);
      if (feedbackResult.success) {
        setReportFeedback(feedbackResult.feedback);
      } else {
        setReportFeedback([]);
      }
    } catch (error) {
  ...
  const handleAddFeedback = async (reportId, feedbackText, status = 'reviewed') => {
    try {
      // Determine if this is a lecture report or general report
      const isLectureReport = selectedFeedback && selectedFeedback.course_name;

      let result;
      if (isLectureReport) {
        // Use lecture report feedback method
        result = await feedbackService.submitLectureReportFeedback(reportId, feedbackText, status);
      } else {
        // Use general report feedback method
        result = await feedbackService.submitReportFeedback(reportId, feedbackText, status);
      }
                  ...
                  <div className="modal-content large-modal">
                    ...
                    <div className="modal-body">
                      <div className="report-details">
                        ...
                        <div className="detail-row">
                          <strong>Scheduled Time:</strong> {selectedReport.scheduled_time ? new Date(selectedReport.scheduled_time).toLocaleString() : 'Not specified'}
                        </div>
                        <div className="detail-row">
                        ...
                        <div className="detail-row">
                          <strong>Status:</strong> <span className={`status-badge ${selectedReport.status?.toLowerCase() || 'pending'}`}>{selectedReport.status || 'Pending'}</span>
                        </div>
                        {reportFeedback.length > 0 && (
                          <div className="detail-row">
                            <strong>Feedback:</strong>
                            <div className="feedback-list">
                              {reportFeedback.map((feedback, index) => (
                                <div key={index} className="feedback-item">
                                  <p>{feedback.feedback_text}</p>
                                  <small>Status: {feedback.status} • {new Date(feedback.created_at).toLocaleDateString()}</small>
                                </div>
                              ))}
                            </div>
                          </div>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (feedbackText.trim()) {
                            handleAddFeedback(selectedFeedback.id, feedbackText, status);
                          } else {
