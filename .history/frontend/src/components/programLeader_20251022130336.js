...
const ProgramLeaderDashboard = ({ user, onLogout }) => {
  ...
  const handleSubmitClass = async () => {
    ...
    try {
      ...
      const formattedScheduledTime = classFormData.scheduled_time.replace('T', ' ') + ':00';

      const result = await classService.createClass({
        course_id: parseInt(classFormData.course_id),
        class_name: classFormData.class_name,
        lecturer_id: parseInt(classFormData.lecturer_id),
        venue: classFormData.venue || null,
        scheduled_time: formattedScheduledTime,
        total_registered_students: parseInt(classFormData.total_registered_students) || 0
      });
