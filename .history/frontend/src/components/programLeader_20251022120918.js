...
const ProgramLeaderDashboard = ({ user, onLogout }) => {
  ...
  const handleSubmitClass = async () => {
    if (!classFormData.course_id || !classFormData.class_name || !classFormData.lecturer_id) {
      alert('Course, class name, and lecturer are required.');
      return;
    }
    ...
    try {
      const result = await classService.createClass({
        course_id: classFormData.course_id,
        class_name: classFormData.class_name,
        lecturer_id: classFormData.lecturer_id,
        venue: classFormData.venue,
        scheduled_time: classFormData.scheduled_time,
        total_registered_students: classFormData.total_registered_students
      });
