...
// ==================== LECTURE REPORTS ROUTES (Proxy to /api/reports) ====================

// Get all lecture reports (proxy to /api/reports)
app.get('/lecture-reports', async (req, res) => {
  let connection;
  try {
    connection = await pool.getConnection();
    const [reports] = await connection.execute(
...
// Get single lecture report by ID (proxy to /api/reports/:id)
app.get('/lecture-reports/:id', async (req, res) => {
  let connection;
  try {
    const { id } = req.params;
    connection = await pool.getConnection();
...
// Create new lecture report (proxy to /api/reports)
app.post('/lecture-reports', async (req, res) => {
  let connection;
  try {
    const { title, content, authorId, authorName } = req.body;
...
// Update lecture report (proxy to /api/reports/:id)
app.put('/lecture-reports/:id', authenticateToken, async (req, res) => {
  let connection;
  try {
    const { id } = req.params;
    const { title, content } = req.body;
...
// Delete lecture report (proxy to /api/reports/:id)
app.delete('/lecture-reports/:id', authenticateToken, async (req, res) => {
  let connection;
  try {
    const { id } = req.params;
    connection = await pool.getConnection();
