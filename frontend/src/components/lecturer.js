...
const Dashboard = ({ user, onLogout, isDarkTheme, toggleTheme }) => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      setIsLoading(true);
      try {
        // Fetch principal lecturer first as it's needed for filtering classes
        await fetchPrincipalLecturer();
  ...
  const fetchClasses = async () => {
    try {
      ...
      if (response.success) {
        // Filter classes assigned to the current lecturer AND created by the principal lecturer
        const lecturerClasses = response.classes.filter(
          cls => cls.lecturer_id === user.id && cls.principal_lecturer_id === principalLecturer?.id
        );
  ...
  const fetchReports = async () => {
    try {
      ...
      if (response.success) {
        // If principal lecturer, show all reports; otherwise, filter for current author
        const userReports = user.role === 'principal-lecturer'
          ? response.reports
          : response.reports.filter(report => report.author_id === user.id);
      ...
      <div className="dashboard-main">
        ...
        <div className="dashboard-content">
          ...
          {activeTab === 'classes' && (
            <div className="tab-content animated-fadeIn">
              <div className="content-header">
                <h2>🎓 Classes Created by Principal Lecturer</h2>
                <p>View classes created by the principal lecturer</p>
              </div>
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="detail-grid">
                ...
                <div className="detail-item">
                  <label>Course:</label>
                  <span>{modules.find(m => m.course_id === selectedClass.course_id)?.course_name || 'N/A'}</span>
                </div>
                <div className="detail-item">
                  <label>Scheduled Time:</label>
                  <span>{new Date(selectedClass.scheduled_time).toLocaleString()}</span>
                </div>
                <div className="detail-item">
                  <label>Venue:</label>
                ...
                <div className="detail-item">
                  <label>Total Registered Students:</label>
                  <span>{selectedClass.total_registered_students || 0}</span>
                </div>
                <div className="detail-item">
                  <label>Enrolled Students:</label>
                  <span>{selectedClass.enrolled_count || 0}</span>
                </div>
              </div>
            </div>
            <div className="modal-footer">
              <button className="btn-outline" onClick={() => setShowClassModal(false)}>Close</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="detail-grid">
                ...
                <div className="detail-item">
                  <label>Author:</label>
                  <span>{viewingReport.author_name}</span>
                </div>
                <div className="detail-item">
                ...
                <div className="detail-item">
                  <label>Status:</label>
                  <span className={`status ${viewingReport.status.toLowerCase()}`}>
                    {viewingReport.status}
                  </span>
                </div>
                <div className="detail-item full-width">
                  <label>Content:</label>
                  <span>{viewingReport.content}</span>
                </div>
              </div>
            </div>
            <div className="modal-footer">
              <button className="btn-outline" onClick={() => setViewingReport(null)}>Close</button>
            </div>
          </div>
        </div>
      )}

      {/* Lecture Report Details Modal */}
      {viewingLectureReport && (
        <div className="modal-overlay" onClick={() => setViewingLectureReport(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>Lecture Report Details</h3>
              <button className="modal-close" onClick={() => setViewingLectureReport(null)}>×</button>
            </div>
            <div className="modal-body">
              <div className="detail-grid">
                <div className="detail-item">
                  <label>Faculty Name:</label>
                  <span>{viewingLectureReport.faculty_name}</span>
                </div>
                <div className="detail-item">
                  <label>Class Name:</label>
                  <span>{viewingLectureReport.class_name}</span>
                </div>
                <div className="detail-item">
                  <label>Week of Reporting:</label>
                  <span>{viewingLectureReport.week_of_reporting}</span>
                </div>
                <div className="detail-item">
                  <label>Date of Lecture:</label>
                  <span>{new Date(viewingLectureReport.date_of_lecture).toLocaleDateString()}</span>
                </div>
                <div className="detail-item">
                  <label>Course Name:</label>
                  <span>{viewingLectureReport.course_name}</span>
                </div>
                <div className="detail-item">
                  <label>Course Code:</label>
                  <span>{viewingLectureReport.course_code}</span>
                </div>
                <div className="detail-item">
                  <label>Lecturer Name:</label>
                  <span>{viewingLectureReport.lecturer_name}</span>
                </div>
                <div className="detail-item">
                  <label>Actual Students Present:</label>
                  <span>{viewingLectureReport.actual_students_present}</span>
                </div>
                <div className="detail-item">
                  <label>Total Registered Students:</label>
                  <span>{viewingLectureReport.total_registered_students}</span>
                </div>
                <div className="detail-item">
                  <label>Venue:</label>
                  <span>{viewingLectureReport.venue}</span>
                </div>
                <div className="detail-item">
                  <label>Scheduled Time:</label>
                  <span>{viewingLectureReport.scheduled_time}</span>
                </div>
                <div className="detail-item full-width">
                  <label>Topic Taught:</label>
                  <span>{viewingLectureReport.topic_taught}</span>
                </div>
                <div className="detail-item full-width">
                  <label>Learning Outcomes:</label>
                  <span>{viewingLectureReport.learning_outcomes}</span>
                </div>
                <div className="detail-item full-width">
                  <label>Recommendations:</label>
                  <span>{viewingLectureReport.recommendations || 'None'}</span>
                </div>
              </div>
            </div>
            <div className="modal-footer">
              <button className="btn-outline" onClick={() => setViewingLectureReport(null)}>Close</button>
            </div>
          </div>
        </div>
      )}
