...
const StudentDashboard = ({ user, onLogout }) => {
  ...
  const fetchLecturers = async () => {
    try {
      setApiLoading(true);
      // Use the correct endpoint for lecturers (users with lecturer role)
      ...
      if (response.success && response.lecturers) {
        const lecturersWithRatings = response.lecturers.map(lecturer => ({
          ...
          courses: getLecturerCourses(lecturer.role),
  ...
  const fetchLectureReports = async () => {
    try {
      ...
      if (response.success) {
        setLectureReports(response.lectureReports);
      }
    } catch (error) {
  ...
  const submitRating = async () => {
    if (selectedLecturer && rating > 0 && selectedCourse) {
      try {
        ...
        // Check if student has already rated this lecturer
        const checkResponse = await ratingService.checkRating({
          lecturer_id: selectedLecturer.id,
          student_id: user.id
        });

        if (checkResponse.alreadyRated) {
          alert('You have already rated this lecturer.');
          return;
        }
