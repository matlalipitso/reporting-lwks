...
import '../App.css';
import { authService, ratingService, classService, lectureReportService } from '../services/authservice';
...
const StudentDashboard = ({ user, onLogout }) => {
  ...
  const fetchLecturers = async () => {
    try {
      setApiLoading(true);
      const response = await authService.get('/api/lecturers-users');
  ...
  const updateLecturerCourses = (coursesData) => {
    // This would ideally come from a course_assignments table
    // For now, we'll assign courses randomly or based on lecturer role
    setLecturers(prevLecturers => 
      prevLecturers.map(lecturer => {
        let assignedCourses = [];
        
        if (lecturer.role === 'principal-lecturer') {
          assignedCourses = coursesData
            .filter(course => course.course_code.includes('4') || course.course_code.includes('5'))
            .slice(0, 3)
            .map(course => `${course.course_code} - ${course.course_name}`);
        } else {
          assignedCourses = coursesData
            .filter(course => course.course_code.includes('1') || course.course_code.includes('2') || course.course_code.includes('3'))
            .slice(0, 4)
            .map(course => `${course.course_code} - ${course.course_name}`);
        }
        ...
        if (assignedCourses.length === 0) {
          assignedCourses = ['CS101 - Introduction to Programming', 'CS201 - Data Structures'];
        }
  ...
  // Check if student has already rated a lecturer
  const checkIfRated = async (lecturerName) => {
    try {
  ...
  const submitRating = async () => {
    if (selectedLecturer && rating > 0 && selectedCourse) {
      try {
        ...
        const alreadyRated = await checkIfRated(selectedLecturer.name);
        if (alreadyRated) {
          alert('You have already rated this lecturer. You can update your rating.');
        }

        // Extract course code from selected course
        const courseCode = selectedCourse.split(' - ')[0];
        const courseName = selectedCourse.split(' - ')[1] || selectedCourse;
                  ...
                  <div className="rating-form">
                    ...
                    <div className="course-input">
                      ...
                      <select
                        ...
                        required
                      >
                        <option value="">Choose a course you took with this lecturer...</option>
