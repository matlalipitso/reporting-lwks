...
const ProgramLeaderDashboard = ({ user, onLogout }) => {
  ...
  // View Class functionality state
  ...
  // Edit Class functionality state
  ...
  const [editClassFormLoading, setEditClassFormLoading] = useState(false);
  ...
  const handleSubmitLecturer = async () => {
    if (!lecturerFormData.fullName || !lecturerFormData.email || !lecturerFormData.password) {
    ...
    try {
      const result = await authService.register({
        fullName: lecturerFormData.fullName,
        email: lecturerFormData.email,
        password: lecturerFormData.password,
        role: lecturerFormData.role,
  ...
  const handleSubmitClass = async () => {
    if (!classFormData.course_id || !classFormData.class_name || !classFormData.lecturer_id || !classFormData.scheduled_time) {
      alert('Course, class name, lecturer, and scheduled time are required.');
      return;
    }

    setClassFormLoading(true);
    try {
      // Format the scheduled_time to MySQL DATETIME format (YYYY-MM-DD HH:MM:SS)
      const date = new Date(classFormData.scheduled_time);
      if (isNaN(date.getTime())) {
        alert('Please enter a valid scheduled time.');
        return;
      }
      const formattedScheduledTime = date.toISOString().slice(0, 19).replace('T', ' ');

      const courseId = parseInt(classFormData.course_id);
      const lecturerId = parseInt(classFormData.lecturer_id);
      const totalStudents = classFormData.total_registered_students ? parseInt(classFormData.total_registered_students) : 0;
      ...
      if (isNaN(lecturerId) || lecturerId <= 0) {
        alert('Please select a valid lecturer.');
        return;
      }
      ...
      const result = await classService.createClass({
        ...
        venue: classFormData.venue.trim() === '' ? null : classFormData.venue.trim(),
      ...
      if (result.success) {
        ...
        // Reload classes
        const classesResult = await classService.getAllClasses();
        if (classesResult.success) setClasses(classesResult.classes || []);
      } else {
  ...
  const handleEditClass = (cls) => {
    ...
    setEditClassFormData({
      ...
      venue: cls.venue || '',
      scheduled_time: cls.scheduled_time ? new Date(cls.scheduled_time).toISOString().slice(0, 16) : '',
      total_registered_students: cls.total_registered_students || ''
    });
  ...
  const handleSubmitEditClass = async () => {
    if (!editClassFormData.course_id || !editClassFormData.class_name || !editClassFormData.lecturer_id || !editClassFormData.scheduled_time) {
      alert('Course, class name, lecturer, and scheduled time are required.');
      return;
    }

    setEditClassFormLoading(true);
    try {
      // Format the scheduled_time to MySQL DATETIME format (YYYY-MM-DD HH:MM:SS)
      const date = new Date(editClassFormData.scheduled_time);
      if (isNaN(date.getTime())) {
        alert('Please enter a valid scheduled time.');
        return;
      }
      ...
      const courseId = parseInt(editClassFormData.course_id);
      const lecturerId = parseInt(editClassFormData.lecturer_id);
      const totalStudents = editClassFormData.total_registered_students ? parseInt(editClassFormData.total_registered_students) : 0;
      ...
      const result = await classService.updateClass(selectedClassForEdit.class_id, {
        course_id: courseId,
        class_name: editClassFormData.class_name.trim(),
        lecturer_id: lecturerId,
        venue: editClassFormData.venue.trim() === '' ? null : editClassFormData.venue.trim(),
        scheduled_time: formattedScheduledTime,
        total_registered_students: totalStudents
      });
              ...
              <div className="monitoring-section">
                ...
                <div className="list-container">
                  ...
                  {feedbacks.map(feedback => (
                    <div key={feedback.feedback_id} className="list-row">
                      <span className="name-cell">{feedback.report_title}</span>
                      ...
                      <span>
                        <span className={`priority-badge ${feedback.priority?.toLowerCase() || 'medium'}`}>
                          {feedback.priority || 'Medium'}
                        </span>
                      </span>
                      <span>
                        <span className={`status-badge ${feedback.status?.toLowerCase() || 'submitted'}`}>
          ...
          {activeTab === 'classes' && (
            <div className="tab-content">
              <div className="section-header">
                ...
                <div className="header-actions">
                  <button className="btn-primary" onClick={() => setShowAddClassModal(true)}>+ Add New Class</button>
                  <button className="btn-outline">Export</button>
                </div>
              </div>
              ...
              <div className="list-container">
                ...
                {classes.map(cls => (
                  <div key={cls.class_id} className="list-row">
                    ...
                    <span>
                      <button className="btn-sm" onClick={() => handleEditClass(cls)}>Edit</button>
                      <button className="btn-sm btn-outline" onClick={() => handleViewClass(cls)}>View</button>
                    </span>
                  </div>
                ))}
                {classes.length === 0 && (
          ...
          {activeTab === 'lectures' && (
            <div className="tab-content">
              <div className="section-header">
                ...
                <div className="header-actions">
                  <button className="btn-primary" onClick={() => setShowAddLecturerModal(true)}>+ Add Lecturer</button>
                  <button className="btn-outline">Export</button>
                </div>
              </div>
      ...
      {showAddClassModal && (
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header">
              <h3>Add New Class</h3>
              <button className="close-btn" onClick={() => setShowAddClassModal(false)}>×</button>
            </div>
            <div className="modal-body">
              <div className="form-group">
                <label>Course</label>
                <select
                  ...
                  onChange={(e) => setClassFormData({...classFormData, course_id: e.target.value})}
                >
              ...
              <div className="form-group">
                ...
                <select
                  value={classFormData.lecturer_id}
                  onChange={(e) => setClassFormData({...classFormData, lecturer_id: e.target.value})}
                >
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) => setClassFormData({...classFormData, venue: e.target.value})}
              ...
              <div className="form-group">
                ...
                <input
                  type="datetime-local"
                  value={classFormData.scheduled_time}
                  onChange={(e) => setClassFormData({...classFormData, scheduled_time: e.target.value})}
                />
              </div>
              <div className="form-group">
                ...
                <input
                  type="number"
                  value={classFormData.total_registered_students}
                  onChange={(e) => setClassFormData({...classFormData, total_registered_students: e.target.value})}
                  min="1"
